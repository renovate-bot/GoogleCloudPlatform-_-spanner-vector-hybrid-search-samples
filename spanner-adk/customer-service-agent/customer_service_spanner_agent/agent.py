...
from prompts import agent_instruction, nla_agent_instruction
...
nla_agent = LlmAgent(
    ...
    description=(
        "Retrieve order related info for a customer from Spanner database"
    ),
    instruction=nla_agent_instruction,
    tools=[spanner_toolset],
)

nla_agent_tool = agent_tool.AgentTool(agent=nla_agent)

# The variable name `root_agent` determines what your root agent is for the
# debug CLI
root_agent = LlmAgent(
    model="gemini-2.5-flash",
    name="customer_service_spanner_agent",
    description=(
        "Agent to answer questions about Spanner database tables and"
        " execute SQL queries."
    ),
    instruction=agent_instruction,
    tools=[nla_agent_tool],
)